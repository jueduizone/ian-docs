...
AI 正在改变开源依赖关系。开发者可以通过 AI 调用、参考、fork、重写开源项目，却不一定留下 package manifest、import 或 fork history。传统依赖图只能看到显性依赖，看不到 AI 带来的隐性影响。

OpenSeed 第一阶段交付 Pledge Registry、Dependency Funnel、SPARK.md、Public Accountability Dashboard 和 SPARK License v0.1 草案，跑通从项目承诺、依赖追踪、资金分配、维护者认领到公开问责的完整流程。

Ethereum 是第一套 funding and accountability backend。长期目标是让 OpenSeed 成为通用的开源归因与资助标准，服务开源项目、AI 工具、基金会、企业开源办公室和开发者社区。
...
OpenSeed 的起点来自 SPARK Pledge 讨论。SPARK 的核心想法是：商业化成功的项目，应当把一部分未来收益回流给它所依赖的开源生态。

这个方向和 Vitalik 最近提到的 dependency funding 一致：资助真实依赖，而不是随机发 grant。

现在缺的不是理念，而是可运行的机制：谁承诺、依赖谁、钱怎么分、维护者怎么认领、社区如何验证。

OpenSeed 先做参考实现。第一阶段用 Ethereum 跑通资金和公开问责流程，同时把 `SPARK.md`、Dependency Funnel 和 SPARK License 草案做成可被其他生态复用的开放标准。

## 4. 缺口在哪里

| 层级 | 当前状态 | OpenSeed 第一阶段交付 |
| --- | --- | --- |
| SPARK / Dependency Funding 理念 | 已有讨论 | 参考实现 |
| 项目承诺入口 | 缺失 | Pledge Registry |
| 显性依赖追踪 | 部分可见 | Dependency Funnel |
| AI 隐性依赖 | 基本不可见 | SPARK.md + 人工声明入口 |
| 资金分配 | 缺少标准流程 | Allocation Recommendation |
| 维护者认领 | 缺少统一流程 | Claim workflow |
| 公开问责 | 缺失 | Public Accountability Dashboard |
| License 路径 | 缺少草案和评审流程 | SPARK License Draft v0.1 |

## 5. 为什么我们能做

OpenSeed 有三个基础。

第一，团队在 SPARK 早期讨论中。我们不是从外部解读一个概念，而是在机制草案阶段就参与了讨论，理解它为什么要设计成递归承诺和依赖回流。

第二，有真实开发者社区入口。OpenBuild、开源社、PyChina、RustCC 覆盖了华语 Web3、开源、Python、Rust 开发者社区，可以找到第一批愿意试点的项目，也能拿到真实反馈。

第三，有推动 License 的社区和组织基础。开源社是 OSI 首个中国成员，团队成员长期参与 Apache、Linux Foundation、OSI 等开源治理网络。第一阶段要做的是拿出 v0.1 草案，组织社区、基金会、OSPO 和法律顾问评审，形成可继续推进的版本。

OpenSeed 的角色是参考实现和试点组织者，不主张独占 SPARK，也不把 Ethereum 实现绑定为唯一版本。
...
SPARK Seed Pool 是第一阶段的种子资金池，用来跑通真实资助流程。

资金来源包括 EF grant、社区捐赠和合作方支持。后续可以接收商业化项目的 Public Goods Allocation 回流。

资金使用方式：

1. 根据 Dependency Funnel 生成分配建议
2. 由项目确认分配结果
3. 对上游维护者做 outreach
4. 维护者完成认领
5. 通过多签或链上支付完成分发
...
1. `SPARK.md` 接入支持
2. Dependency Report
3. Pledge / Addendum 接入协助
4. Dashboard 展示
5. 首轮 dependency funding 资格
6. 社区和基金会曝光
...
OpenSeed 第一阶段构建的基础设施全部开源。

包括：

1. Registry 代码
2. Dependency Funnel 算法
3. Dashboard 前端
...
| 时间 | 交付 |
| --- | --- |
| 第 1–2 个月 | Registry v0.1、SPARK.md v0.1、Cohort 招募、License 草案启动 |
| 第 3–4 个月 | Dependency Funnel v0.1、Dashboard v0.1、首批 Dependency Report |
| 第 5 个月 | 模拟分配、Pledge Addendum 试用、License v0.1 评审 |
| 第 6 个月 | 第一轮真实资助、Cohort 报告、公开标准文档 |
...
| --- | --- |
| Cohort 项目 | 10–15 个 |
| 完成 SPARK.md 的项目 | 10 个以上 |
| Dependency Report | 10 份以上 |
| 完成模拟分配 | 10 次 |
| 完成真实资助 | 1 轮 |
| 上游依赖映射 | 100 个以上 |
| Addendum 采用项目 | 3–5 个 |
| License Draft 评审组织 | 2 个以上 |
| Public Dashboard | 上线 |

## 14. 团队

**林旅强（Richard Lin）**：OpenSeed 联合创始人。开源社联合创始人，开源社是中国最大的厂商中立开源社区，也是首个加入 OSI 的中国组织。现任 Datastrato.ai 开源生态负责人，曾任华为云和零一万物开发者生态负责人，长期参与 Apache、Linux Foundation、OSI 国际社区。
...
**SPARK 工作组关系**：OpenSeed 团队参与 SPARK Pledge 早期讨论，并会持续向工作组反馈试点数据和社区采用情况。

## 15. 后续扩展：AI Runtime Attribution
