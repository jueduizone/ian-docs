...
**拨款方式：** 按里程碑分期拨款  
**申请方：** OpenSeed  
**主要产出：** 开源软件、开放规范、公开报告，以及一个基于 Ethereum 的 dependency funding 试点

## 1. 执行摘要

OpenSeed 是一套面向 AI 时代的开源归因与资助基础设施。第一阶段从 Ethereum 生态开始试点。
...
Ethereum 用多年时间建立了 crypto 世界里最成熟的公共物品与开源资助文化。下一步，是让资金流更能反映真实依赖关系，更透明，也更容易复用。
...
5. **SPARK Seed Pool Pilot**：小规模真实资助试点，通过可审计的 Ethereum 工作流，把种子资金分发给上游维护者。
6. **SPARK License / Addendum Research**：围绕 pledge 和 AI attribution 的早期法律、治理和社区评审材料。

我们申请 **USD 100,000 / 6 个月**，按里程碑分期拨款，用于完成 v0.1 系统、接入第一批 10–15 个 Cohort 项目、生成公开 Dependency Reports、运行一轮模拟分配、完成一轮小规模真实资助，并把方法论作为面向 Ethereum 生态和更广泛开源生态的开放基础设施发布。
...
SPARK Seed Pool 是一个小规模试点资金池，用来验证完整流程。
...
Ethereum 是 OpenSeed 第一阶段最合适的 funding and accountability backend。
...
4. **可编程资助**：稳定币、智能合约、attestations，以及 Safe、EAS 等现有工具，让 funding workflow 可以在不重造金融栈的前提下被测试。
...
OpenSeed 不把 Ethereum 绑定为唯一实现。Ethereum 是第一阶段试点，因为它同时具备价值观、工具和真实需求。
...
| 里程碑 | 时间 | 拨款 | 交付物 |
| --- | --- | ---: | --- |
| M1：Specification and architecture | 第 1 个月 | $20,000 | 技术架构、SPARK.md v0.1 draft、registry schema、cohort selection criteria、公开项目 repo |
| M2：Registry and SPARK.md tooling | 第 2 个月 | $20,000 | Pledge Registry v0.1、GitHub / domain / wallet identity flow、SPARK.md templates、validator、GitHub Action / CLI prototype |
| M3：Dependency Funnel and first reports | 第 3–4 个月 | $25,000 | Dependency Funnel v0.1，覆盖 npm、PyPI、Cargo、Go、Maven；5+ preliminary Dependency Reports；methodology notes |
| M4：Dashboard and simulation round | 第 5 个月 | $20,000 | Public Accountability Dashboard v0.1、10+ cohort reports、simulated allocation round、SPARK Pledge Addendum v0.1 |
| M5：Real distribution and final report | 第 6 个月 | $15,000 | 一轮小规模真实资金分发、maintainer claim workflow、public cohort report、SPARK.md spec、Dependency Funnel methodology、License compatibility memo |
| **合计** | **6 个月** | **$100,000** |  |
...
| 类别 | 金额 | 说明 |
| --- | ---: | --- |
| Registry and Ethereum attestations | $18,000 | Pledge Registry、身份绑定、EAS / attestation 集成、数据模型、可审计性 |
| Dependency Funnel | $22,000 | Package ecosystem 集成、依赖展开、GitHub mapping、scoring methodology、report generation |
...
Grant 资助的工作会开源和公开。Seed distribution 占比刻意较小，因为第一阶段重点是验证流程。
...
15. 供其他生态 fork 或复用的 workflow 文档
...
| 完成真实资助分发 | 1 |
| 联系 maintainer | 30+ |
| 成功 claim funding 的 maintainer | 5+ |
...
**林旅强（Richard Lin）**：OpenSeed 联合创始人。开源社联合创始人，开源社是中国最大的厂商中立开源社区之一，也是首个加入 OSI 的中国组织。现任 Datastrato.ai 开源生态负责人，曾任华为云和零一万物开发者生态负责人，长期参与 Apache、Linux Foundation、OSI 等开源社区。
...
**社区基础**：OpenBuild、开源社、PyChina 和 RustCC 覆盖华语 Web3、开源、Python 和 Rust 开发者社区，可以帮助识别试点项目、收集 maintainer 反馈，并传播最终方法论。
...
| Maintainer 不认领资金 | 第一轮金额保持小规模，使用多种联系渠道，支持 GitHub Sponsors / OpenCollective / Ethereum wallet |
...
Ethereum 是最适合验证这套机制的地方。它有公共物品文化，有工具，有真实依赖面。AI 时代让这个问题更紧迫，因为开源影响正在变得更不可见，而越来越多软件正在基于已有开源工作被生成。

通过一笔 **USD 100,000、按里程碑分期拨款** 的 grant，OpenSeed 可以交付 v0.1 系统，接入第一批 Cohort，发布 dependency reports，跑通一轮真实资助试点，并把方法论作为面向 Ethereum 和更广泛开源生态的开放基础设施发布。
